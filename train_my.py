...
from models.unet_3d_condition__shared import UNet3DConditionModel
...
from utils.dataset import VideoJsonDataset, SingleVideoDataset, \
    ImageDataset, VideoFolderDataset, CachedDataset,VideoFolderDataset2,VideoFolderDatasetUCF
...
def get_train_dataset(dataset_types, train_data, tokenizer):
    ...
    for DataSet in [VideoJsonDataset, SingleVideoDataset, ImageDataset, VideoFolderDataset,VideoFolderDataset2,VideoFolderDatasetUCF]:
    ...
    if len(train_datasets) > 0:
        ...
        raise ValueError("Dataset type not found: 'json', 'single_video', 'folder', 'image','text_folder','UCF_folder'")
...
def load_primary_models(pretrained_model_path):
    ...
    unet = UNet3DConditionModel.from_pretrained(pretrained_model_path, subfolder="unet",low_cpu_mem_usage=False,ignore_mismatched_sizes=True)
...
def main(
    ...
    shared_diffusion_steps: Tuple[int] = [],
    **kwargs
):

    *_, config = inspect.getargvalues(inspect.currentframe())
    config1 = ProjectConfiguration(project_dir=".", logging_dir="loggerfolder3")
    accelerator = Accelerator(
        gradient_accumulation_steps=gradient_accumulation_steps,
        mixed_precision=mixed_precision,
        log_with=LoggerType.TENSORBOARD,
        project_config=config1
    )
    config2 = {
        "num_iterations": 10000,
        "learning_rate": 1e-5,
    }
    accelerator.init_trackers("example_project", config=config2)
    ...
    unet.conv_out_shared.weight = \
        Parameter(unet.conv_out.weight.repeat(2, 1 ,1,1))
    unet.conv_out_shared.bias = \
        Parameter(unet.conv_out.bias.repeat(2))
    ...
    for i in range(len(shared_diffusion_steps)-1):
        result.append([shared_diffusion_steps[i], shared_diffusion_steps[i+1]])
    progress_bar = tqdm(range(global_step, max_train_steps * len(result) ), disable=not accelerator.is_local_main_process)
    progress_bar.set_description("Iter")
    iter = 1

    def finetune_unet(batch, train_encoder=False,shared_diffusion_multiple=0,shared_diffusion_step=None):
        nonlocal use_offset_noise
        nonlocal rescale_schedule
        nonlocal text_encoder
        nonlocal double_frame_flag
...
##################   frame_scale为帧的缩放尺度，0-500步为1，500到750为2，750到875为4

        frame_scale = 2 ** shared_diffusion_multiple if shared_diffusion_step[1] != 1000 else 2**(shared_diffusion_multiple-1)
        video_length = latents.shape[2]
        num_frames = video_length // frame_scale
        # init_frame= random.randrange(0,video_length-num_frames,1)
        # latents = latents[:,:,init_frame:(init_frame + num_frames-1),:,:]
        latents = latents[:,:,0:num_frames,:,:]
        # Get video length
        video_length = latents.shape[2]
        ...
        if shared_diffusion_step == None:
            timesteps = torch.randint(0, noise_scheduler.num_train_timesteps, (bsz,), device=latents.device)
            # Add noise to the latents according to the noise magnitude at each timestep
            # (this is the forward diffusion process)
            noisy_latents = noise_scheduler.add_noise(latents, noise, timesteps)
        else:
            timesteps =  torch.randint(shared_diffusion_step[0], shared_diffusion_step[1], (bsz,), device=latents.device)
            noisy_latents = noise_scheduler.add_noise(latents, noise, timesteps)
            _,c,f,h,w = noisy_latents.shape
            noisy_latents_half = torch.zeros(bsz,c,int(f/2),h,w).to(torch.device("cuda"))
            for index in range(int(f/2)):
                noisy_latents_half[:,:,index,:,:] = noisy_latents[:,:,2*index,:,:] + noisy_latents[:,:,2*index + 1,:,:]
        ...
        for i in range(2):
            ...
            if should_truncate_video and i == 1 and not double_frame_flag :
                noisy_latents = noisy_latents[:,:,1,:,:].unsqueeze(2)
                target = target[:,:,1,:,:].unsqueeze(2)
            elif i == 1 and  double_frame_flag:
                continue
            ...
            if double_frame_flag:
                model_pred = unet(noisy_latents_half, timesteps, encoder_hidden_states=encoder_hidden_states,double_frame_flag=double_frame_flag).sample
            else:
                model_pred = unet(noisy_latents, timesteps, encoder_hidden_states=encoder_hidden_states,double_frame_flag=double_frame_flag).sample
        ...
        loss = losses[0] if len(losses) == 1 else (losses[0] + losses[1])/2
        nonlocal iter
        iter = iter + 1 
        logs = {"step_loss": loss.detach().item(), "lr": lr_scheduler.get_last_lr()[0]}
        # accelerator.log({"training_loss": loss.detach().item()}, step=step)
        progress_bar.set_postfix(**logs)
        # tensorboard_tracker.log({"training_loss": loss}, step=iter)
        accelerator.log({"training_loss": loss.detach().item()}, step=iter)
        # double_frame_flag = not double_frame_flag
        return loss, latents
                    ...
                    for i in range(len(result)):
                        with accelerator.autocast():
                            loss, latents = finetune_unet(batch, train_encoder = train_text_encoder,
                                                          shared_diffusion_step = result[i],shared_diffusion_multiple = i)
                        ...
                        try:
                            ...
                            progress_bar.update(1)
                            # accelerator.log({"train_loss": loss}, step=global_step*len(result))
