...
class VideoFolderDataset2(Dataset):
    def __init__(
        self,
        tokenizer=None,
        width: int = 256,
    ...
    @staticmethod
    def __getname__(): return 'text_folder'
    ...
    def __getitem__(self, index):
        ...
        # else:
        #     prompt = self.fallback_prompt
        prompt = self.video_files[index].split("/")[-1].split(".")[0]

        prompt_ids = self.get_prompt_ids(prompt)

        return {"pixel_values": normalize_input(video[0]), "prompt_ids": prompt_ids[0], "text_prompt": prompt, 'dataset': self.__getname__()}

class VideoFolderDatasetUCF(Dataset):
    def __init__(
        self,
        tokenizer=None,
        width: int = 256,
        ...
        **kwargs
        ...
        self.video_files = glob(f"{path}/*/*.avi")
    ...
    @staticmethod
    def __getname__(): return 'UCF_folder'
    ...
    def __getitem__(self, index):
        ...
        # else:
        #     prompt = self.fallback_prompt
        prompt = self.video_files[index].split("/")[-1].split("_")[0]
